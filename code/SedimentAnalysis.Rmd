...
fedoc <- read_csv("../data/raw_data/ASL 10feb22.csv")
...
write.csv(fedoc_filt, "../data/processed_data/10feb22_formatted.csv", row.names = F)
...
jpeg("../figures/Fe_OC_pct_7dec21.jpg", width = 6, height = 5, units = "in", res = 300)
...
p =calcs_exp%>%
  ...
  geom_text(data = sig, aes(label = Labels))+
  ylab("Fe-OC as a percent of total sediment OC")+
  xlab("Treatment")+
...
jpeg("../figures/OC_pct.jpg", width = 6, height = 5, units = "in", res = 300)
Labels<- c(NA,"a","a","b","b")
Experiment_treatment<- c("Day 0","Anoxic","Anoxic to Oxic","Oxic to Anoxic","Oxic")
C_pct_init = 5.3
...
jpeg("../figures/Fe_OC_total_7dec21.jpg", width = 6, height = 5, units = "in", res = 300)
calcs_exp%>%
  ggplot(aes(x = Experiment_treatment, y = Fe_OC_per_unit_sed, shape = as.factor(Experiment_sample_date)))+
  geom_point()+
  ylab("Fe-OC (umol per g sediment)")+
  xlab("Treatment")+
  labs(shape = "Sample date")+
  theme_bw()
dev.off()
...
calcs_anova = calcs_exp%>%
  filter(Experiment_sample_date>0)
anova_pct = aov(lm(calcs_anova$Fe_OC_pct~calcs_anova$Experiment_treatment))
summary(anova_pct)
TukeyHSD(anova_pct)
...
calcs_res = calcs%>%
  ...
  mutate(Reservoir = tolower(str_extract(Sample_ID, "^(f|b|F|B)")),
         Date_collected = as.Date(str_extract(Sample_ID, "[0-9]+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[0-9]+"), format = "%d%b%y"),
...
calcs_res%>%
  ...
  ylab("Sediment OC (% of sediment mass)")+
  xlab("")+
  geom_point()+
  facet_grid(~Year, scales = "free_x")+
  theme_bw()
...
calcs_res%>%
  ...
  ylab("Fe bound OC (µmol/g sediment)")+
  xlab("")+
  geom_point()+
  facet_grid(~Year, scales = "free_x")+
  theme_bw()

jpeg("../figures/Sed_to_sed_trap_comparison.jpg", width = 6, height = 4, units = "in", res = 300)
calcs_res%>%
  ...
  ggplot(aes(x = Sed_trap, y = Fe_OC_pct, color = Reservoir))+
  #geom_hline(yintercept = 21.5)+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(position=position_jitterdodge(dodge.width=0.75, jitter.width = 0.1),aes(group=Reservoir, shape = year))+
  ylab("Fe-OC as a percent of total sediment OC")+
  xlab("")+
  scale_x_discrete(labels=c("TRUE" = "Sedimentation Trap", "FALSE" = "Sediment"))+
  theme_bw()+
  labs(shape = "Year")+
  #geom_text(aes(y = 21.5, x = T,label = "Lalonde et al. (2012)"), vjust = 0, nudge_y = 1, hjust = 1, nudge_x = 0.6)+
...
summary(aov(stats$Fe_OC_pct~stats$Reservoir*stats$year))
stats = calcs_res%>%
  filter(!is.na(C_pct_init),
         is.na(Sample_ID)|Sample_ID!="b21sep21r2_")%>%
  mutate(year = year(Date_collected))
summary(aov(stats$C_pct_init~stats$Reservoir*stats$year))
...
calcs_res%>%
  ...
  facet_wrap(~year)+
  geom_jitter(position=position_jitterdodge(dodge.width=0.75, jitter.width = 0.2),aes(group=year, y = Fe_OC_pct), data = calcs_res%>%
  filter(!is.na(Fe_OC_pct),
         Fe_OC_pct>0,
         Sed_trap==F,
         is.na(Sample_ID)|Sample_ID!="b21sep21r2_")%>%
...
jpeg("../figures/FCR_year_comp.jpg", width = 6, height = 4, units = "in", res = 300)
calcs_res%>%
  filter(!is.na(Fe_OC_pct),
         Fe_OC_pct>0,
         Sed_trap==F,
  ...
  geom_boxplot(outlier.shape = NA)+
  ylab("Fe bound OC (% of sediment OC)")+
  xlab("")+
  labs(color = "Year")+
...
jpeg("../figures/FCR_year_comp_mass.jpg", width = 6, height = 4, units = "in", res = 300)
calcs_res%>%
  filter(!is.na(Fe_OC_per_unit_sed),
         Fe_OC_pct>0,
         Sed_trap==F,
...
jpeg("../figures/FCR_year_comp_C_pct.jpg", width = 6, height = 4, units = "in", res = 300)
calcs_res%>%
  filter(!is.na(C_pct_init),
         Sed_trap==F,
         is.na(Sample_ID)|Sample_ID!="b21sep21r2_",
...
calcs_res%>%
  filter(Fe_OC_pct>50,!Sed_trap)

calcs_res%>%
  filter(Date_collected==as.Date("2019-08-05"))

on <- as.Date(c("2019-07-08", "2019-08-05"))
off <- as.Date(c("2019-07-18", "2019-08-19"))
...
calcs_res%>%
  ...
  geom_text(aes(x = on+1, y = 3, label = "on", hjust = 0),data = on_off)+
...
calcs_res%>%
  ...
  geom_text(aes(x = on+1, y = 15, label = "on", hjust = 0),data = on_off)+
  geom_point(size = 2, shape = 21, fill = "#4B4E6D")
dev.off()
```
