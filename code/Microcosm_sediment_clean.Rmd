...
calcs_exp = calcs_exp%>%
  filter(Experiment_sample_date%in%c(20,23)) #Only analyzing differences between treatments at the end of the experiment (days 20 and 23)

calcs_exp$Experiment_treatment<- factor(calcs_exp$Experiment_treatment, levels = c("ex","aa","ao","oa","oo"), labels = c("Day 0","Hypoxic","Hypoxic to oxic","Oxic to hypoxic","Oxic"))

levene_test(calcs_exp, Fe_OC_pct~Experiment_treatment)
levene_test(calcs_exp, C_pct_init~Experiment_treatment)
levene_test(calcs_exp, Fe_OC_per_unit_sed~Experiment_treatment)
...
#prep labels
Labels<- c(NA,"a","a","b","b")
Experiment_treatment<- c("Day 0","Hypoxic","Hypoxic to oxic","Oxic to hypoxic","Oxic")
...
ggpubr::ggarrange(feoc_total, oc_pct,feoc_pct, nrow = 1, labels = "auto", common.legend = T, legend = "none")
dev.off()
```