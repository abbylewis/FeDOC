...
```{r}
#Load data
calcs_raw = read.csv("../data/processed_data/Calculations_26May22.csv", na = c("","NA"))
...
calcs_trimmed = calcs_raw%>%
  ...
  filter(is.na(Date_collected)|
           !(Date_collected == as.Date("2019-09-02")&Rep%in%c(3,4)| #spilled
               ...
               Date_collected == as.Date("2019-10-07")&Rep%in%c(2)| #spilled
               ...
               Date_collected == as.Date("2021-06-10")))%>% #Labeling got messed up for these samples-> had to remove from dataset)
...
calcs = calcs_trimmed%>%
  pivot_wider(id_cols = c(Sample_type, Experiment_treatment, Experiment_sample_date, Rep, Reservoir, Date_collected, Sed_trap), values_from = C_pct, names_from = Type, names_prefix = "C_pct_")%>%
