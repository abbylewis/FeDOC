...
on_off = data.frame(on,off,Reservoir="FCR")%>%
  mutate(Year = year(on),
         center_on = (off-on)/2+on,
         center_off = (on-lag(off))/2+lag(off))
...
ggpubr::ggarrange(feoc_total,test_tot,
          c_pct,test_c,
...
jpeg("../figures/All_sed_data_2019_boxplot.jpg", width = 6, height = 3, units = "in", res = 300)
ggpubr::ggarrange(test_tot,test_c,test_pct, nrow = 1, 
          labels = c("a","b","c"))+
  theme(plot.margin = unit(c(1,0,0,0), "lines"))
dev.off()

jpeg("../figures/All_sed_data_2019_timeseries.jpg", width = 6, height = 7.5, units = "in", res = 300)
ggpubr::ggarrange(feoc_total,c_pct,feoc_pct, nrow = 3,
          labels = c("a","b","c"),
          label.y = 1.05)+
  theme(plot.margin = unit(c(1,0,0,0), "lines"))
dev.off()
