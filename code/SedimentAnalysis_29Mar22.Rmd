...
full = calcs_exp%>%
  rename(Treatment = Experiment_treatment,
         Sample_day = Experiment_sample_date)%>%
  mutate(Sample_day=as.numeric(Sample_day),
         Rep = as.numeric(Rep))%>%
  select(-Type)%>%
  full_join(full)

full$Treatment[full$Treatment == "Anoxic to Oxic"] <- "Anoxic to oxic"
full$Treatment[full$Treatment == "Oxic to Anxic"] <- "Oxic to Anoxic"
...
facets = c("DOC (mg/L)", " Total iron (mg/L)","DO (mg/L)","C_pct_init", "Fe_OC_per_unit_sed","Fe_OC_pct")
names(facets) = c("NPOC","Iron","DO_mgL","C_pct_init", "Fe_OC_per_unit_sed","Fe_OC_pct")

f = full%>%
  filter(filt == "Unfiltered")%>%
  mutate(Iron = Conc/1000)%>%
  pivot_longer(cols = c(NPOC,Iron,DO_mgL,C_pct_init, Fe_OC_per_unit_sed,Fe_OC_pct))%>%
...
jpeg("../figures/combined_exp_graph_extra.jpg", width = 8, height = 6, units = "in",res = 300)
f
dev.off()

facets = c("DOC (mg/L)", "Soluble Fe (mg/L)","Total Fe (mg/L)","DO (mg/L)","C_pct_init", "Fe_OC_per_unit_sed","Fe_OC_pct")
names(facets) = c("NPOC", "Filtered","Unfiltered","DO_mgL","C_pct_init", "Fe_OC_per_unit_sed","Fe_OC_pct")

formatted = full%>%
  mutate(Iron = Conc/1000)%>%
  pivot_wider(names_from = filt,
              values_from = c(Iron))%>%
  pivot_longer(cols = c(NPOC, Unfiltered,Filtered,DO_mgL,C_pct_init, Fe_OC_per_unit_sed,Fe_OC_pct))%>%
  filter(!is.na(Status))

formatted$name <- factor(formatted$name, levels = c("DO_mgL", "NPOC", "Unfiltered", "Filtered","C_pct_init", "Fe_OC_per_unit_sed","Fe_OC_pct"))
...
jpeg("../figures/combined_exp_graph_full_extra.jpg", width = 6, height = 9, units = "in",res = 300)
f
dev.off()
...
jpeg("../figures/OC_sedTrap.jpg", width = 6, height = 4, units = "in", res = 300)
calcs_res%>%
  filter(Sed_trap)%>%
...
jpeg("../figures/Fe_OC_total_sedTrap.jpg", width = 6, height = 4, units = "in", res = 300)
calcs_res%>%
  filter(Sed_trap)%>%
