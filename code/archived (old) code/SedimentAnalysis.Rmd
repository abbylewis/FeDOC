...
calcs_res%>%
  ...
  ylab("Fe-OC as a percent of total sediment OC")+
  geom_text(aes(x = 1.5, y = 47, label = "***"), size = 8, vjust = 0.5)+
  xlab("")+
...
ggarrange(blank, blank, blank,
          f_mass+ylab("")+ylim(mass_lim),
          f_c+ylab("")+ylim(c_lim),
          f_pct+ylab("")+ylim(pct_lim),
          mass, c, pct, 
          b_mass+
            ylab("")+
          ...
          b_pct+ylab("")+
            theme(plot.background = element_rect(fill='transparent', color=NA))+
            ylim(pct_lim), 
          nrow = 4, ncol = 3, labels = c(NA, NA, NA, "a","b","c",NA,NA,NA,"d",'e',"f"), heights = c(0.1,1,0,1), label.x = 0.25, label.y = 1.1, legend = "none")
...
stats%>%
  ggplot(aes(x = as.factor(year(Date_collected)),y= C_pct_init))+
  geom_point()
...
f_pct = calcs_res%>%
  ...
  xlab("")+
  labs(color = "Year")+
  geom_text(aes(x = 1.5, y = (pct_lim[2]-pct_lim[1])*.9+pct_lim[1], label = "***"), size = 8, vjust = 0.5)+
...
f_c = calcs_res%>%
  ...
  xlab("")+
  scale_color_manual(values = c("#217DAB","#000000"))+
  geom_text(aes(x = 1.5, y = (c_lim[2]-c_lim[1])*.9+c_lim[1], label = "***"), size = 8, vjust = 0.5)+
  labs(color = "Year")+
  geom_jitter(position=position_jitter(width = 0.1), aes(color = year))+
  theme(legend.position = "none")+
  theme_bw()+
...
jpeg("../figures/All_sed_data_res.jpg", width = 10, height = 6, units = "in", res = 300)
ggarrange(ggarrange(f_mass+ylim(mass_lim),
          f_c+ylim(c_lim),
          f_pct+ylim(pct_lim), nrow = 1, legend = "none", labels = c("a","b","c")),ggarrange(
