...
Section 6: Create a heatmap of pH in FCR and BVR
...
This section calculates volume-weighted hypolimnetic oxygen concentrations in falling creek and beaverdam reservoirs. Inputs to this section are the dataframe created in section 1, as well as bathymetry from both reservoirs. Outputs are volume-weighted DO from both reservoirs (.csv files)
```{r}
#Load bathymetry data
inUrl1  <- "https://pasta-s.lternet.edu/package/data/eml/edi/939/2/f7fa2a06e1229ee75ea39eb586577184" 
infile1 <- tempfile()
try(download.file(inUrl1,infile1,method="curl"))
if (is.na(file.size(infile1))) download.file(inUrl1,infile1,method="auto")
...
vol_fcr = read.csv("../data/processed_data/FCR Bathymetry.csv")
...
vol_bvr <- dt1%>%
  ...
  mutate(Depth_m = Depth_m-3) #BVR was 3 m below full pond during this study
...
Section 4: Load DOC and iron data
This section loads data for DOC and iron in water column samples from EDI. These data will be used in the following code chunks
```{r}
#Load DOC data
...
Section 5: Graph Fe, DOC, and oxygen (Figure 4)
```{r}
#IRON
