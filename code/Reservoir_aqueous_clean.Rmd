...
do = dt1%>%
  ...
  filter(!is.na(DO_mgL),
         ...
         Date != as.Date("2019-07-18")|Reservoir=="FCR") #Removing one cast from BVR in 2018 that is mislabeled as FCR
...
```{r}
#Load DOC data
chem <- read_csv('../data/processed_data/chemistry_2013_2021.csv') #2013-2020
...
metals_lowest = metals%>%
  ...
  mutate(Week = as.numeric(format(DateTime, format = "%V")),
         Year = year(DateTime))
#%>%
#group_by(Week, Year, Reservoir)%>%
  #summarize(TFe_mgL=mean(TFe_mgL, na.rm = T))
  
metals_lowest%>%
  filter(Year == 2019,
         month(DateTime)>5,
         month(DateTime)<10)%>%
  ggplot(aes(x = DateTime, y = TFe_mgL))+
  geom_vline(aes(xintercept = on), data= on_off, color = "grey60")+
  geom_vline(aes(xintercept = off),lty=2, data = on_off, color = "grey60")+
  geom_line()+
...
to.plot.do = df.final.new%>%
  ...
  rename(value = DO_mgL,
         DateTime = Date)%>%
  mutate(name = "DO_mgL",
         Reservoir = factor(Reservoir, levels = c("FCR","BVR")))

fe_plot = metals_lowest%>%
  filter(Year == 2019,
         month(DateTime)>5,
         month(DateTime)<10,
         !is.na(TFe_mgL))%>%
  mutate(value = TFe_mgL,
         name = "TFe_mgL",
         Reservoir = factor(Reservoir, levels = c("FCR","BVR")))%>%
  full_join(to.plot.do%>%
              mutate(value = value*5))%>%
  ggplot(aes(x = DateTime, y = value,color = name, size = name))+
  geom_line()+
  ylab("Total Fe (mg/L)")+
  scale_size_manual(values = c(0.5,1.5))+
  scale_color_manual(values = c("grey20","blue"))+
  scale_y_continuous(sec.axis = sec_axis(trans = ~./5, name = "Dissolved oxygen (mg/L)"))+
...
doc_plot = doc_lowest%>%
  ...
  scale_size_manual(values = c(0.5,1.5))+
  scale_color_manual(values = c("grey20","blue"))+
  scale_y_continuous(sec.axis = sec_axis(trans = ~., name = "Dissolved oxygen (mg/L)"))+
  facet_wrap(~Reservoir, scales = "free_x")+
  theme_bw()+
  theme(axis.title.y.left = element_text(colour = "blue"),
        axis.ticks.y.left = element_line(color = "blue"),
...
jpeg("../figures/DOC and Fe line plots.jpg", res = 300, width = 6, height = 5, units = "in")
egg::ggarrange(doc_plot,fe_plot,ncol = 1, labels = c("a","b"))
dev.off()


ggplot(aes(x = Date, y = DO_mgL))+
  geom_line()+
  geom_vline(aes(xintercept = on), data= on_off, color = "grey20")+
  geom_vline(aes(xintercept = off),lty=2, data = on_off, color = "grey20")+
  facet_wrap(~Reservoir)+
  theme_bw()

jpeg("../Figures/PACF_Fe.jpg",width = 6, height = 6, units = "in",res= 300)
par(mfrow = c(2,2))
metals_lowest2 = metals_lowest%>%
  filter(Year == 2019,
         Week>15,
         Reservoir == "FCR")

pacf(metals_lowest2$TFe_mgL[!is.na(metals_lowest2$TFe_mgL)])
...
do_weekly = df.final.new%>%
  mutate(Week = as.numeric(format(Date, format = "%V")),
         Year = year(Date))%>%
  group_by(Week, Year, Reservoir)%>%
  summarize(DO_mgL=mean(DO_mgL, na.rm = T))

metals_lowest_comb = metals_lowest2%>%
  left_join(do_weekly)

metals_lowest_comb$DO_mgL = approx(metals_lowest_comb$Week, metals_lowest_comb$DO_mgL, xout = metals_lowest_comb$Week)$y
  
summary(lm(metals_lowest_comb$TFe_mgL~lag(metals_lowest_comb$TFe_mgL)+log(metals_lowest_comb$DO_mgL)))
...
metals_lowest%>%
  ...
  ggplot(aes(y = TFe_mgL, x = DateTime))+
  geom_point(aes(color = Reservoir, shape = Reservoir))+
  theme_bw()+
  #geom_text(aes(x = do_lim[2]/2, y = fe_lim[2]*9/10, label = "Spearman's R = -0.8\np<0.001"), size = 3)+
  scale_color_manual(values = c("#4B4E6D","#80B192"))+
...
jpeg("../Figures/PACF_DOC.jpg",width = 6, height = 6, units = "in",res= 300)
par(mfrow = c(2,2))
doc_lowest2 = doc_lowest%>%
  filter(Year == 2019,
         Reservoir == "FCR")

pacf(doc_lowest2$DOC_mgL[!is.na(doc_lowest2$DOC_mgL)])
...
lowest_comb = doc_lowest_comb%>%
  full_join(metals_lowest, by = c("Reservoir", "Site", "DateTime", "Depth_m"))

metals_lowest_comb%>%
  filter(year(DateTime)%in%c(2019,2021),
         month(DateTime)>5, 
         month(DateTime)<11)%>%
  arrange(DateTime)%>%
  ...
  mutate(avg_o2 = (DO_mgL+lag(DO_mgL))/2,
         date_dif = DateTime-lag(DateTime),
         #delta_DOC = DOC_mgL-lag(DOC_mgL),
         delta_Fe = TFe_mgL-lag(TFe_mgL))%>%
  filter(!is.na(date_dif)&date_dif>1&date_dif<20)%>%
  ggplot(aes(x = avg_o2, y = delta_Fe/as.numeric(date_dif)))+
  geom_point()
...
on_off = data.frame(on,off,Reservoir="FCR")%>%
  mutate(Year = year(on))%>%
  mutate(Reservoir=factor(Reservoir, levels = c("FCR","BVR")))

#Load data
chem <- read_csv('../data/processed_data/chemistry_2013_2021.csv')
...
doc_bvr_2019_interp <- interp2xyz(interp(doc_bvr_2019$DateTime,doc_bvr_2019$Depth_m,doc_bvr_2019$DOC_mgL,xo = seq(min(doc_bvr_2019$DateTime), max(doc_bvr_2019$DateTime), .5),yo = seq(min(doc_bvr_2019$Depth_m), max(doc_bvr_2019$Depth_m), by = .01), duplicate = "mean"),data.frame = T)

#DOC heatmap
jpeg("../figures/DOC_heatmap_2019.jpeg",width = 8, height = 4, units = "in", res = 300)
doc_hm = doc_fcr_2019_interp%>%
  mutate(Reservoir = "FCR")%>%
  ...
  labs(y = "Depth (m)", title = "Dissolved Organic Carbon",fill=expression('DOC (mg/L)'))+
...
fe_hm = fe_fcr_2019_interp%>%
  ...
  geom_raster(aes(fill = z))+
  scale_y_reverse(expand = c(0,0))+
  labs(title = "     Total Iron ",fill=expression('Fe (mg/L)'))+
  scale_fill_gradientn(colours = blue2green2red(100), na.value="gray")+
...
jpeg("../figures/Fe_DOC_heatmap_2019.jpeg",width = 8, height = 6, units = "in", res = 300)
egg::ggarrange(doc_hm,fe_hm, labels = c("a","b"), ncol = 2)
dev.off()
```


Section 6: pH heatmap. This section creates a heatmap of whole-ecosystem pH data for the supplemental info
```{r}
#Load data
df.final2 = df.final%>%
...
for(year in years){
  ...
  do_bvr<- df.final2%>%
    ...
    filter(Year == year,
           ...
           Depth_m <= 11)
  do_fcr_interp <-
    interp2xyz(interp(do_fcr$Date,do_fcr$Depth_m,do_fcr$pH,xo = seq(min(do_fcr$Date), max(do_fcr$Date),1 ),yo = seq(min(do_fcr$Depth_m), max(do_fcr$Depth_m), by = .01), duplicate = "mean"),data.frame = T)%>%
    mutate(Reservoir = "FCR")
  do_bvr_interp <- interp2xyz(interp(do_bvr$Date,do_bvr$Depth_m,do_bvr$pH,xo = seq(min(do_bvr$Date), max(do_bvr$Date), 1),yo = seq(min(do_bvr$Depth_m), max(do_bvr$Depth_m), by = .01), duplicate = "mean"),data.frame = T)%>%
    mutate(Reservoir = "BVR")
  interp_do = interp_do%>%
    full_join(do_fcr_interp)%>%
    full_join(do_bvr_interp)
...
#DO heatmap
jpeg("../figures/pH_heatmap_2019_2021.jpeg",width = 8, height = 4, units = "in", res = 300)
interp_do%>%
  mutate(x = as.Date(x, origin = "1970-01-01"),
         Year = year(x))%>%
  ggplot(aes(x=x, y=y))+
  ...
  scale_y_reverse(expand = c(0,0))+
  labs(x = "", y = "Depth (m)")+
  scale_fill_gradientn(colours = rev(blue2green2red(100)), na.value="gray", name = "pH")+
  #geom_vline(aes(xintercept = on), data= on_off)+
  #geom_vline(aes(xintercept = off),lty=2, data = on_off)+
  scale_x_date(expand = c(0,0))+
  theme(panel.border = element_rect(fill = NA))+
