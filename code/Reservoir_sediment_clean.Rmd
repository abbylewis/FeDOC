...
feoc_pct = calcs_res%>%
  ...
  scale_color_manual(values = c("#000000","#000000","#217DAB","#000000"))+
  geom_boxplot(outlier.shape = NA, aes(color = res_year))+
  ylab("Fe-OC (% of sediment OC)")+
  xlab("")+
  geom_jitter(width = 0.1,aes(color = res_year))+
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 30, vjust = 1, hjust = 1),
        axis.title.x = element_blank())
...
feoc_sed = calcs_res%>%
  ...
  mutate(year = as.factor(year(Date_collected)),
         res_year = factor(paste(year,Reservoir), levels = c("2019 b", "2021 b","2019 f", "2021 f"), labels = c("BVR 2019","BVR 2021","FCR 2019","FCR 2021")))%>%
  ggplot(aes(x = res_year, y = Fe_OC_per_unit_sed, color = res_year))+
  geom_text(aes(y = mass_lim[2]+1/10*(mass_lim[2]-mass_lim[1]), x = "FCR 2019", label = "n.s."), hjust = 1)+
  scale_color_manual(values = c("#000000","#000000","#217DAB","#000000"))+
  geom_boxplot(outlier.shape = NA)+
  ylab("Fe-OC (µmol/g sediment)")+
  xlab("")+
...
#Combine figures
jpeg("../figures/All_sed_data_res_12May22.jpg", width =6, height =3, units = "in", res = 300)
ggarrange(feoc_sed,c_pct,feoc_pct, nrow = 1)
dev.off()
```
